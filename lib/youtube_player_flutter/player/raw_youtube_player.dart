...
class _RawYoutubePlayerState extends State<RawYoutubePlayer>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.paused:
        _cachedPlayerState = youtubeController.playerState;
        controller?.pause();
        break;
      default:
    }
      ...
      child: InAppWebView(
        ...
        initialOptions: InAppWebViewGroupOptions(
          ...
          android: AndroidInAppWebViewOptions(
            ...
            useHybridComposition: youtubeController.flags.useHybridComposition,
          ),
        ),
        onWebViewCreated: (webController) {
          youtubeController.updateWebControler(webController);
          ...
          webController
              ...
              callback: (args) {
                switch (args.first as int) {
                  case -1:
                    ...
                    youtubeController.changeStatusVideo(PlayerState.unStarted);
                    break;
                  case 0:
                    widget.onEnded?.call(youtubeController.metaData);
                    youtubeController.changeStatusVideo(PlayerState.ended);
                    break;
                  case 1:
                    youtubeController.changeStatusVideo(PlayerState.playing);
                    youtubeController.togglePlaying(true);
                    break;
                  case 2:
                    youtubeController.changeStatusVideo(PlayerState.paused);
                    youtubeController.togglePlaying(false);
                    break;
                  case 3:
                    youtubeController.changeStatusVideo(PlayerState.buffering);
                    break;
                  case 5:
                    youtubeController.changeStatusVideo(PlayerState.cued);
                    break;
                  default:
            ...
            // Dùng cho tùy chọn tốc độ video
            ..addJavaScriptHandler(
            ...
            ..addJavaScriptHandler(
              handlerName: 'Errors',
              callback: (args) {
                youtubeController.updateCodeError(args.first as int);
              },
            )
            ..addJavaScriptHandler(
              handlerName: 'VideoData',
              callback: (args) {
                youtubeController.toggleThumb(false);
                youtubeController.updateMetaData(args.first);
              },
            )
            ..addJavaScriptHandler(
              ...
              callback: (args) {
                ...
                youtubeController.updateTimeLine(
                    valuePosition: Duration(milliseconds: position.floor()),
                    valueBuffer: buffered.toDouble());
              },
            );
        },
        onLoadStop: (_, __) {
          ...
          _onLoadStopCalled = true;
          if (_isPlayerReady) {
            youtubeController.toggleReady(true);
          }
        },
      ),
    );
        ...
        <script>
            ...
            function onYouTubeIframeAPIReady() {
                player = new YT.Player('player', {
                    height: '100%',
                    width: '100%',
                    videoId: '${youtubeController.urlId}',
                    playerVars: {
                        'controls': 0,
                        'playsinline': 1,
                        'enablejsapi': 1,
                        ...
                        'iv_load_policy': 3,
                        'modestbranding': 1,
                        'cc_load_policy': ${boolean(value: youtubeController.flags.enableCaption)},
                        'cc_lang_pref': '${youtubeController.flags.captionLanguage}',
                        'autoplay': ${boolean(value: youtubeController.flags.autoPlay)},
                        'start': ${youtubeController.flags.startAt},
                        'end': ${youtubeController.flags.endAt}
                    },
                    events: {
  ...
  String get userAgent => youtubeController.flags.forceHD
